...
The top-level review session alone writes the map and final review JSON. Workers submit findings through `agentic_code_review_append_finding` and return evidence plus generated IDs. The extension serializes bug/finding-file updates; neither workers nor the top-level review session should read-modify-write it while submissions can still arrive. After all actual worker reports settle, the review session preserves its arrays and sets the final coverage status. Use existing file tools for the review-session-owned records and read everything back before completion. If interrupted, leave progress honest; never fill in a safe grade just to finish.
...
| `coverage` | Agent/zone, expected and received reports, searched scope, depth, stopping reason, unexplored frontier and failed/queued/omitted work. The collection-stage record also carries `pack`: the `sparsity_scan` pack path, its `metrics.json` values (`elapsedMs`, `anchors`, `edges`, `unresolvedRate`, `tokens`) and the index/program notes, or `pack: null` with `collection: "diff-only"` when the index was skipped. |
...
- `kind`: `bug`, `question`, `fix` or `nit`. Bugs must be proven. A `question` is for the author: its answer changes the assessment and the review could not settle it from available evidence; `humanReadable` is the question as you would ask it, `impact` what changes with the answer, `suggestedFix` what would settle it. Questions, fixes and nits are always nonblocking.
...
- `sourceIds`, `requirementIds`, `unitIds`: nonempty lists linking the supplied or reported evidence inventory. Keep the cited records in your report for the review session to accept into the map.
...
Only after worker submissions have settled and the top-level session has finished their returned work, preserve the bug-file envelope and both arrays and set its `status` to `complete`. That final state rejects further submissions. An empty list with explicit evidence limits means no bug was established by this review, not that every possible bug was disproved. Fixing is a separate authorized task. Do not spawn a writer or change bug statuses to resolved. A follow-up's evidence-backed assessment that an earlier bug is fixed belongs in the new map/review, not the sealed historical handoff.

## Final review JSON

Preserve prepared metadata and add:

- `status`: `prepared` until the assessment finishes, then `complete`; include `completedAt` only at completion. Historical `incomplete` artifacts remain readable but are never produced by a new run.
...
- `findings`: map/bug/optional-finding IDs with reconciliation corrections where needed; `commentDrafts`: one entry per accepted finding, `{ findingId, path, line, kind, lens, blocks, body }`, where `body` is the exact complete Markdown block from `pr-comments.md` that the calling agent will render and may later post unchanged; no accepted finding is omitted. `coverage`: inspected zones, stage/report IDs, remaining frontier, check results and completeness.
- `timing`: `{ stages: { "prepare": ms, "review": ms, "synthesize": ms }, totalMs }` computed from the `stage` records at the end; `null` until then.
...
Complete means the review finished, not that the change passed or that evidence has no limits. An unmet criterion can produce a complete, risky review. Missing evidence is stated as an uncertainty or a question; the top-level session finishes every available evidence path and asks once when a decision-changing input requires the user. It never turns an internal helper failure into a public incomplete state.
...
The tool replaces each topic's current block and appends the delta to the history with the run and revision. It preserves older entries and makes identical retries harmless. Neither workers nor the top-level review session directly edits shared notes. Read back every returned path and record `learning: { status: "saved", paths }` on the matching codebase entry, and `learning: { reused, refuted, saved }` at the top level of the review JSON. On failure, retain per-run learning, record `learning: { status: "incomplete", error }` and disclose the persistence gap. Do not claim the notes were saved or remove another writer's lock. Writes are atomic per topic, not across the entire set; an identical retry can finish a partial save.
