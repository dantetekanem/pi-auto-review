...
3. Draft a short comment from the finding's `humanReadable`, `rating`, `impact` and technical evidence. Write the opening as you would say it at the author's desk under `voice.md`; expected versus actual behavior, verification state, whether it is new in this PR and counterevidence go in the details block below it, also in plain sentences. The receipt from `craft.md` goes in the details block in one sentence: what was run, read or counted ("I ran `ruby -e ...` and got `[1]`", "I read `persistence.rb:575` at `abc123`", "the loop issues one `upsert!` per task, about 70 for the fixture"). A claim without a receipt is written as a question. Then check the claims under `presentation.md` without changing the required format below. Do not repeat the full report.
...
A `nit` opens with `Nit: ` and stays to one or two lines. A `fix` is a proposal, not a nit: it opens with the observation and an intro line ("Something like this:", "We can just use `update!` here, as it calls `save!`:"), then the proposed code in a fenced block, then one line on what it buys; keep the code block in the posted body and say `Not a blocker` out loud when it can wait, naming the fast-follow if one is wanted. A `question` opens with the question itself and its details block says what would settle it, in one or two sentences: the test to run with the stub removed, the query count, the failing-path test, the tophat recording, the dashboard. Confirmed bugs open with the consequence; when the fix is clear, they may carry the proposed code the same way a `fix` does. Never write `Blocking: ` anywhere in the prose. Use `blocking` or `nonblocking` for `{blockingStatus}`, based on the accepted assessment, not the rating alone. Optional suggestions are always nonblocking. Keep the status and its parentheses bold: `**(blocking)**` or `**(nonblocking)**`.
