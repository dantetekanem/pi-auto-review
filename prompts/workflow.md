...
1. Read the user's context and linked issue for the problem, without adopting the author's solution. When the target is a pull request URL, follow the sibling `pr-context.md`: pull the description, the review threads and comments, and the CI status through the provider's CLI (`gh` for github.com, `gs` for Meteorite), save them in the run directory and record them as requirement and evidence records before anything else. Identify the repository and exact base/head revisions (the PR's base and head shas when there is a PR), or a fingerprint of the local diff including relevant untracked files. Do not modify or reset the checkout.
...
If the run context contains the line `mode: diff-only`, skip the index on purpose: record `collection: "diff-only"` and `pack: null` in the intake decision, the collection coverage record and the final review JSON, then follow the collection prompt as written below. This exists so runs with and without the pack can be compared; do not choose it on your own. Otherwise start collection with one `sparsity_index` call when the tool is available: pass the exact base/head range (or `worktree` for a local diff), `output_dir` set to this run's artifact directory, and the default depth. The pack reads the changed files at the range head; when the checkout is at another commit the manifest carries a `revision:` line saying so, and nobody re-reads the changed files through `git show`. Read `metrics.json` from that directory and copy `elapsedMs`, `anchors`, `edges`, `unresolvedRate` and `tokens` into the collection coverage record and the final review JSON as `pack`, with `collection: "pack"`. Say "Indexing code changes for agents" in the intake record, then read the manifest and the anchor sections. The pack is evidence: changed definitions with fingerprints, resolved edges with their labels, callers, tests, definitions removed with surviving callers, and an explicit recap of unresolved edges and the frontier. Read it this way: each `anchor:<id>` section is the diff view for one changed definition, with `+|` on added or modified lines and `removed (old lines …)` blocks with `-|` for what the change deleted; `changed lines (+)` gives the exact ranges. A caller marked `(changed in this diff)` is another anchor, so trace the pair together. `tests: none reference this symbol by name` means untested by name, not untested. An edge marked outside the root names the missing namespace; widen `root` for that one anchor only when the dependency bears on a criterion. The recap is the gap list: unresolved edges with reasons, skipped anchors, the frontier and untested anchors go into coverage records as stated unknowns. Pass the pack path, the spilled sections' line ranges, and each worker's anchor IDs in its mission. Collectors verify and extend the pack (unresolved edges that matter, files the index listed as other or generated, contracts static tracing cannot see); they do not re-derive what it already contains. If the tool is unavailable or fails, record that gap and fall back to the collection prompt.

With a pack in hand, decide from its manifest whether anyone needs to collect at all. The pack is the accepted inventory, and stages 2 and 3 spawn nobody, when all of these hold: `truncated: none`; no skipped production anchor; the unresolved edges that remain do not sit inside an anchor a criterion depends on; and no file the pack listed as other or generated carries a contract a criterion depends on. In that case write the units straight from the anchors (one unit per anchor or per pair of anchors that call each other), write the collection coverage record from the manifest (files, anchors, edges, unresolved rate, frontier, untested anchors), record `collectors: 0`, and go to zone assignment. Zone reviewers verify their own contracts inside their anchors; that is their job, not a collector's.

When a condition fails, the failing items are the gap list, and it is closed: each entry names one unresolved edge, one frontier definition, one skipped anchor or one other/generated file, and the criterion it bears on. Give that list to one `read-collect` lane with the collection prompt, launched in the same batch as the zone reviewers, never before them; reviewers do not wait for it, and its report is reconciled in stage 5. Give the deep-collection prompt to a lane only for entries that need a connected trace past the pack's depth (an async hop, a host/engine boundary, a dynamic dispatch); otherwise record stage 3 as skipped with the reason. A collector never receives "build the inventory": the pack did that in seconds. Without a pack (`diff-only` or tool failure), follow the collection prompt as before, then the deep-collection prompt, in sequence.

A follow-up reuses prior evidence under the contract above instead of spawning agents solely to repeat a stage. Preserve source evidence, not just summaries. Search misses with partial coverage are unknowns, not proof of absence.
...
Taste observations are leads, and `taste.md` says which ones pay off and how to follow them. Follow a lead before judging it: surrounding guards, full methods, callers, tests, the off path. Reconcile findings with the ticket, existing threads and automated comments. A blocker must be introduced, expanded or depended on by this change; label pre-existing risk honestly. Never block on taste alone; block on what it uncovered.

Prove claims through source traces and safe focused checks. Distinguish a confirmed defect, a question the author can answer, a coverage gap and an optional suggestion. Every material finding needs evidence, impact and the smallest useful fix or proof needed. Submit confirmed bugs immediately through `agentic_code_review_append_finding` for a later fixing agent; do not wait until the final summary. A question whose answer changes the assessment and which the review could not settle in budget is a `question` finding, submitted with its lens, not a private uncertainty. A suggested fix or nit is nonblocking and stays separate from confirmed bugs.

## Reconcile and grade

Internally check each criterion again after collection, deep tracing and final review: requirement → expected behavior → implementation → evidence → connected impact. Do not adapt the requirement to match the solution. Resolve disagreements with evidence, not voting or another whole-PR review. Investigate remaining concrete questions within the run's budget; unresolved material uncertainty affects the final grade. Questions submitted for the author stay nonblocking; an unanswered question that gates the assessment is a coverage gap, not a defect.
...
Save the final records using the artifact contract and verify that they can be read back. For runs with `codebasesRoot`, save source-linked codebase notes after assessment, read back their paths and record any persistence failure. Finish the tracked stages honestly, then use `report_and_exit` for one report led by the human-only PR summary. Write it under `voice.md` and check it under `presentation.md`, no longer than needed and never over 200 words. Keep grades, reasons, evidence and artifact paths outside that prose. Actionable blockers belong in separate source-line drafts, not a general blocking PR body; nothing is published by this run. If saving fails, disclose it and do not claim the review is fully complete. Stop. Later clarification or code changes require a separately requested run.

## Write plainly

Every sentence a person will read follows `voice.md`: written as you would say it to the author, then checked against the evidence under `presentation.md`. Say what breaks and when before the technical detail. Keep file locations and reproduction evidence precise. Do not pad findings or repeat the report in every comment. Put detailed traces in the map, not a long final response.
