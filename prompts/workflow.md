...
Save the final records using the artifact contract and verify that they can be read back. For runs with `codebasesRoot`, save source-linked codebase notes after assessment, read back their paths and record any persistence failure. Finish the tracked stages honestly, then use `report_and_exit` for one report led by the human-only PR summary. Write it in two passes under `presentation.md`, no longer than needed and never over 200 words. Keep grades, reasons, evidence and artifact paths outside that prose. Actionable blockers belong in separate source-line drafts, not a general blocking PR body; nothing is published by this run. If saving fails, disclose it and do not claim the review is fully complete. Stop. Later clarification or code changes require a separately requested run.

## Write plainly

Follow `presentation.md` for the optional user-voice skill or prompt and humanizer pass: concrete cause and effect, familiar words, no filler, hype or unnecessary jargon. Explain "what breaks and why" before technical detail. Keep file locations and reproduction evidence precise. Do not pad findings or repeat the report in every comment. Put detailed traces in the map, not a long final response.
