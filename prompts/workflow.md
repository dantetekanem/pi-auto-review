...
1. Check `PI_PROVIDER/PI_MODEL` and `PI_REASONING_LEVEL` against `preflight`. On a mismatch, ask the user in this session to select the preflighted model and `medium`, then resume; do not end the review. The review session itself runs at `medium`.
