...
- What to fix, ask or improve: a short decision summary of accepted bugs, questions, nits and optional fixes, separating what blocks from what is a fast-follow. Proposals with code are named here and shown in full in the comment blocks. When there is no finding, list what each lens checked instead, so the reader can trust the nothing.
- What is good: one to three specific lines on what the change does well (a test that pins the whole mapping, a rule moved to its owner, a clean failure path). Skip the section rather than invent praise.
...
- How I looked and what I could not see: the exact revision, the contracts and failure cases examined, checks run and their results, the receipts behind the claims (what was run, read or counted), untested areas; one sentence on how context was gathered (the pack's anchors, edges, unresolved rate and build time, whether a collector ran and for which gaps, or diff-only); one sentence on what earlier reviews contributed and what this run added to the codebase notes; the measured total time with the largest stage, from `timing`.
