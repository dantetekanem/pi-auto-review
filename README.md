...
The visible review session uses its preflighted model at `medium`. It calls `spawn_swarm_agents` from pi-extended-teams once, with `completion_group: { delivery: "all-settled" }`, for at most two `read-review` lanes and one `read-collect` lane. The configured tiers run both at `medium`. `read-collect` gets a closed list of criterion-bearing scan gaps; `read-review` judges a bounded responsibility zone and runs every taste lens. Lanes report through `report_and_exit`; the grouped report resumes the review session when every lane settles. A lane that ends partial, blocked, failed or stopped leaves its items to the review session. There is no status polling, inbox loop, second batch or stronger tier.
