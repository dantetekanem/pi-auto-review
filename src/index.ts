...
function prepareReview(
  ...
  if (!activeTools.includes('spawn_agent') && !activeTools.includes('spawn_swarm_agents')) {
  ...
  const launch = readPrompt('launch');
  ...
  const mission = launch
    .replace('{{coordinator}}', () => JSON.stringify(prompts.coordinator))
...
function requestCoordinator(
  ...
  return new Promise((resolve, reject) => {
    ...
    const interrupted = () => finish(new Error(
      `Review launch acknowledgement was interrupted; a coordinator may still start. Do not retry this run. Check ${coordinator.name}. Artifacts: ${paths.review}`,
    ));
    ...
    unsubscribe = pi.events.on(responseEvent, data => {
      ...
      const response = data as {
        ...
        ok?: unknown;
        error?: unknown;
        details?: { queued?: unknown };
      };
      if (response.requestId !== runId || response.type !== 'spawn_agent') return;

      if (response.ok === true && response.details) {
        finish(undefined, response.details.queued === true);
      } else {
        finish(new Error(
          `Review launch failed: ${String(response.error ?? 'invalid teams response').slice(0, 1000)}. Artifacts: ${paths.review}`,
        ));
      }
    });
...
export function registerReview(pi: ExtensionAPI, root = join(getAgentDir(), 'auto-review')): void {
  ...
  const launch = async (context: string, ctx: ExtensionContext, signal?: AbortSignal) => {
    ...
    return {
      content: [{
        type: 'text' as const,
        text: `Review coordinator ${coordinator.name} ${queued ? 'queued' : 'started'}. Its report will arrive through pi-extended-teams. Artifacts: ${paths.review}`,
      }],
      details: {
        runId,
        sessionId,
        paths,
        coordinator: { ...coordinator, queued },
      },
    };
  };
  ...
  pi.registerTool({
    ...
    async execute(_id, { context = '' }, signal, _onUpdate, ctx) {
      return launch(context, ctx, signal);
    },
  });
}
